...
def express(url):
    ...
    cnt=0
    span=soup.find_all('h4')
    for i in span:
       url2='http://www.express.co.uk'
       topic=i.text
       ref_=i.find_parent('a')['href']
       ref='http://www.express.co.uk'+ref_
       print(ref)
       ...
       country='UK'
       news='EXPRESS'
       #genre=soup2.find('ul',attrs={'class':'page sub-nav'}).find('li',attrs={'class':'active'}).text
       if('finance' in url):
           genre='Business'
       elif('news' in url):
           genre='World'
           ...
           genre='Sport'
       print(genre)
       date_=soup2.find('time').find_all('span')
       for d in date_[1:]:
           date=d.text
       date=dated(date)
       cnt=cnt+1
       ...
       tab=soup2.find_all('div',attrs={'text-description'})
       context=''
       for x in tab:
...
#express('https://www.express.co.uk/sport')
#express('https://www.express.co.uk/news')       
express('http://www.express.co.uk/finance')
